...
dataset_path = f"{abs_path}/dataset/{OCCLUSION_BOUNDS[0]}_{OCCLUSION_BOUNDS[1]}"
temp_path = f"{abs_path}/temp"

print(f"Started with occlusion bounds set as {OCCLUSION_BOUNDS}.")
...
for root, dirs, files in os.walk(temp_path):
    if USE_RGB:
        print(f"{global_index} pairs ready. \t"
              f"{int(round(all_files / 2, 0))} pairs in current directory to check. \t"
